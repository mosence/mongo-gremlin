...
public class MongodbGraphStepStrategyTest{
    ...
    public void doTest() {
        ...
        strategies.addStrategies(MongodbGraphStepStrategy.instance());
    ...
    private static GraphTraversal.Admin<?, ?> g_V(final Object... hasKeyValues) {
        ...
        final MongodbGraphStep<Vertex, Vertex> graphStep = new MongodbGraphStep<>(new GraphStep<>(traversal, Vertex.class, true));
    ...
    private static GraphStep<?, ?> V(final Object... hasKeyValues) {
        final MongodbGraphStep<Vertex, Vertex> graphStep = new MongodbGraphStep<>(new GraphStep<>(EmptyTraversal.instance(), Vertex.class, true));
        for (int i = 0; i < hasKeyValues.length; i = i + 2) {
            graphStep.addHasContainer(new HasContainer((String) hasKeyValues[i], (P) hasKeyValues[i + 1]));
        }
        return graphStep;
    ...
    public static Iterable<Object[]> generateTestParameters() {
        ...
        return Arrays.asList(new Object[][]{
                ...
                {__.V().as("a").dedup().has("name", "marko").or(has("age", 10), has("age", gt(32))).filter(has("name", "bob")).has("lang", "java"),
                        g_V("name", eq("marko"), "lang", eq("java"), "name", eq("bob"), "age", eq(10).or(gt(32))).dedup().as("a"), TraversalStrategies.GlobalCache.getStrategies(MongodbGraph.class).toList()},
                {__.V().has("name", "marko").or(not(has("age")), has("age", gt(32))).has("name", "bob").has("lang", "java"),
                        g_V("name", eq("marko"), "name", eq("bob"), "lang", eq("java")).or(not(filter(properties("age"))), has("age", gt(32))), TraversalStrategies.GlobalCache.getStrategies(MongodbGraph.class).toList()},
                ...
                {__.V().out().V().has("age", gt(32)).barrier(10).has("name", "marko").as("a"), g_V().out().barrier(LAZY_SIZE).asAdmin().addStep(V("age", gt(32), "name", eq("marko"))).barrier(LAZY_SIZE).barrier(10).as("a"), TraversalStrategies.GlobalCache.getStrategies(MongodbGraph.class).toList()},
        });
    }
}